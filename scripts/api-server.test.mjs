...
import { analyzePayload } from "./api-server.mjs";
...
describe("analyzePayload", () => {
  test("rejects empty intake before calling OpenAI", async () => {
    ...
    await expect(
      analyzePayload({ rawIntake: "  " }, { OPENAI_API_KEY: "secret" }, fetchImpl)
    ).resolves.toEqual({
      status: 400,
      body: {
        ok: false,
        error: "Paste an email, form text, screenshot notes, or task details first."
      }
    });
  ...
  test("returns a missing key error without calling OpenAI", async () => {
    ...
    await expect(analyzePayload({ rawIntake: "field trip" }, {}, fetchImpl)).resolves.toEqual({
      status: 500,
  ...
  test("returns a safe error when OpenAI output is malformed", async () => {
    ...
    await expect(
      analyzePayload({ rawIntake: "field trip" }, { OPENAI_API_KEY: "secret" }, fetchImpl)
    ).resolves.toEqual({
      status: 502,
      body: { ok: false, error: "LifeMap could not analyze this yet. Try again or edit the intake." }
    });
  });
});
